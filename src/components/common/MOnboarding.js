...
export default class extends Component {
    ...
    initState(props, updateIndex = false) {
        ...
        initState.total = initState.children
            ? initState.children.length || 1
            : 0;
    ...
    renderNextButton = () => {
        ...
        // prev condition: this.props.loop || this.state.index !== this.state.total - 1
        if (this.state.index !== this.state.total - 1) {
            button = this.props.nextButton || (
                <Text style={styles.buttonText}>›</Text>
            );
        } else {
            button = this.props.getStartButton || (
                <Text style={styles.buttonText}>›</Text>
            );
        }

        onButtonPress = () => {
            if (this.props.onFinish && this.state.index == this.state.total - 1)
                this.props.onFinish();
        ...
        return (
            <TouchableOpacity
                style={{ position: 'absolute', ...this.props.nextButtonStyle }}
                onPress={() => button !== null && onButtonPress()}
    ...
    renderSkipButton = () => {
        ...
        if (this.props.skipButton && this.state.index != this.state.total - 1) {
            button = this.props.skipButton;
        }

        return (
            <TouchableOpacity
                style={{ position: 'absolute', ...this.props.skipButtonStyle }}
                onPress={() => button !== null && this.props.onFinish()}
                disabled={!this.props.skipButton}>
                <View>{button}</View>
            </TouchableOpacity>
        );
    };
    ...
    renderPrevButton = () => {
        ...
        return (
            <TouchableOpacity
                style={{ position: 'absolute', ...this.props.prevButtonStyle }}
